	...
	<div class="main_right">
		...
		<div class="box">
			...
			<div class="form">
				<table border="0">
  					<tr>
  						<td>学生编号</td><td>学生名称</td><td>登录密码</td><td>所在班级</td><td>学生性别</td><td>注册时间</td><td>学分数</td>
  						<td><input class="btn" type="button" value="添加学生" onclick="window.location.href=('tea/InsertStu.jsp')"/></td>
  					...
  					<td><%= stu.getSclass() %></td>
  					<td><%= stu.getSsex() %></td>
  					<td><%= stu.getPublishTime() %></td>
  					<td><%= stu.getScredit() %></td>
  					<td><input class="btn" type="button" value="修改信息" onClick="window.location.href=('tea/updateStu.jsp?id=<%= stu.getSNo() %>')"></td>
  					<td><input class="btn" type="button" value="删除信息" onClick="window.location.href=('servlet/serDeleteStu?id=<%= stu.getSNo() %>')"></td>
  					</tr>
  					<%} %>
  				</table>
  			<div class="form_1">
